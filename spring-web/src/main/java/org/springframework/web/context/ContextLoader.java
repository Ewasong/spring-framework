...
public class ContextLoader {
		...
		try {
			...
			if (this.context == null) {
				/** 这里创建在ServletContext中存储的根上下文ROOT_WEB_APPLICATION_CONTEXT，同时把它存到ServletContext中去，
				 注意这里使用的ServletContext的属性值是ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE，
				 以后的应用都是根据这个属性值取得根上下文的*/
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		...
		// 如果在ServletContext中配置了需要使用的CONTEXT_CLASS,那就使用这个class，当然前提是这个class是可用的
